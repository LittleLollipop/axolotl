...
public class PersistentGraph {
    ...
    var reverseAdjacencyList: [VertexID: Set<VertexID>] = [:] // For PageRank (incoming edges)
    var propertyIndex: [String: [PropertyValue: Set<VertexID>]] = [:] // propertyName -> (value -> vertex IDs)
    ...
    /// Convenience initializer (in-memory mode, no persistence)
    public convenience init() {
        let tempDir = NSTemporaryDirectory()
        let tempFileName = "axolotl_\(UUID().uuidString).json"
        let tempPath = (tempDir as NSString).appendingPathComponent(tempFileName)
        try! self.init(filePath: tempPath)
    }

    // MARK: - CRUD Operations

    public func addVertex(id: VertexID? = nil, properties: [String: PropertyValue] = [:]) throws -> VertexID {
    ...
    public func addEdge(from: VertexID, to: VertexID, properties: [String: PropertyValue] = [:], weight: Double = 1.0) throws -> EdgeID {
    ...
    public func getNeighbors(of vertexId: VertexID) -> [VertexID] {
        return Array(adjacencyList[vertexId] ?? [])
    }

    public func getStatistics() -> (vertexCount: Int, edgeCount: Int) {
    ...
    public func shortestPath(from start: VertexID, to target: VertexID) -> [VertexID] {
    ...
    public func bfsDistances(from start: VertexID) -> [VertexID: Int] {
        guard vertices[start] != nil else { return [:] }
    ...
    public func sssp(from start: VertexID) -> [VertexID: Int] {
        guard vertices[start] != nil else { return [:] }
    ...
    public func connectedComponents() -> [Set<VertexID>] {
    ...
    public func computeWeaklyConnectedComponents() -> [Set<VertexID>] {
        var visited = Set<VertexID>()
    ...
    public func pageRank(dampingFactor: Double = 0.85, maxIterations: Int = 100, tolerance: Double = 1e-6) -> [VertexID: Double] {
    ...
    /// Returns dictionary mapping vertex ID to PageRank score
    public func incrementalPageRank(dampingFactor: Double = 0.85, maxIterations: Int = 100, tolerance: Double = 1e-6) -> [VertexID: Double] {
        let n = vertices.count
        guard n > 0 else { return [:] }
    ...
    public func getPageRank(dampingFactor: Double = 0.85) -> [VertexID: Double] {
        if !isPageRankDirty {
    ...
    public func betweennessCentrality() -> [VertexID: Double] {
        let n = vertices.count
    ...
    public func getBetweennessCentrality() -> [VertexID: Double] {
        if !isBetweennessDirty {
            return betweennessScores
        }
    ...
    public func detectCommunitiesGreedy(maxIterations: Int = 100) -> [VertexID: Int] {
        let n = vertices.count
        guard n > 0 else { return [:] }
    ...
    public func getCommunitiesGreedy(maxIterations: Int = 100) -> [[VertexID]] {
    ...
    /// - Returns: Dictionary mapping vertex ID to community ID
    public func detectCommunitiesLouvain(maxIterations: Int = 100, resolution: Double = 1.0) -> [VertexID: Int] {
        let n = vertices.count
        guard n > 0 else { return [:] }
    ...
    public func getAllNeighbors(of vertex: VertexID) -> Set<VertexID> {
    ...
    public func getCommunitiesLouvain(maxIterations: Int = 100, resolution: Double = 1.0) -> [[VertexID]] {
    ...
    public func findByProperty(_ propertyName: String, value: PropertyValue) -> [VertexID] {
        return Array(propertyIndex[propertyName]?[value] ?? [])
    }

    /// Get all unique values for a property
    public func getPropertyValues(_ propertyName: String) -> [PropertyValue] {
    ...
    public func exportToDOT(directed: Bool = true, labelProperty: String = "name", coloredByCommunity: Bool = false) -> String {
        var dot = ""
    ...
    public func saveToDOT(filePath: String, directed: Bool = true, labelProperty: String = "name", coloredByCommunity: Bool = false) throws {
    ...
    public func exportWithCommunities(filePath: String) throws {
...
class BinaryBuffer {
    ...
    public func writePropertyValue(_ value: PropertyValue) {
        switch value {
        case .string(let s):
        ...
        case .bool(let b):
            ...
            writeBytes([b ? 1 : 0])
        case .float(let f):
            writeBytes([BinaryFormat.PropertyType.float.rawValue])
            writeDouble(Double(f))
        case .data(let d):
            writeBytes([BinaryFormat.PropertyType.data.rawValue])
            var dataSize = Int64(d.count).bigEndian
            data.append(Data(bytes: &dataSize, count: MemoryLayout<Int64>.size))
            data.append(d)
        case .null:
    ...
    public func readPropertyValue() -> PropertyValue {
        ...
        case .bool:
            ...
            return .bool(b == 1)
        case .float:
            return .float(Float(readDouble()))
        case .data:
            let dataSize = Int(readUInt64())
            let dataBytes = readBytes(dataSize)
            return .data(Data(bytes: dataBytes))
        case .null:
            return .null
        }
    }
