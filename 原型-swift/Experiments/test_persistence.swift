...
// MARK: - Type Definitions (Inline for Testing)
...
class PersistentGraphDatabase {
    ...
    func addVertex(id: VertexID? = nil, properties: [String: PropertyValue] = [:]) throws -> VertexID {
        ...
        guard vertices[vid] == nil else {
            throw GraphError.vertexNotFound(vid)
        }
...
// Test 1: Create new database and add data
print("Test 1: Create new database and add data")
print(String(repeating: "-", count: 50))

do {
    let db = try PersistentGraphDatabase(filePath: testDbPath)
    ...
    // Add edges
    print("\n2. Adding edges...")
    try db.addEdge(from: v1, to: v2, properties: ["type": .string("knows")])
    try db.addEdge(from: v1, to: v5, properties: ["type": .string("knows")])
    try db.addEdge(from: v2, to: v3, properties: ["type": .string("knows")])
    try db.addEdge(from: v3, to: v4, properties: ["type": .string("knows")])
    try db.addEdge(from: v4, to: v5, properties: ["type": .string("knows")])
...
do {
    ...
    try db.addEdge(from: v6, to: 0, properties: ["type": .string("knows")])
    ...
    let db2 = try PersistentGraphDatabase(filePath: testDbPath)
