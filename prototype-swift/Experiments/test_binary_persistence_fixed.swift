...
// MARK: - Binary Buffer Helper (Fixed alignment issues)
...
class BinaryBuffer {
    ...
    // Read methods (Fixed - no alignment issues)
...
print("=== Binary Persistence Test (Fixed) ===\n")
...
print("   Save speedup: \(String(format: "%.2f", jsonSaveTime / binarySaveTime))x (binary faster)")
print("   Load speedup: \(String(format: "%.2f", jsonLoadTime / binaryLoadTime))x (binary faster)")
print("   Size reduction: \(String(format: "%.2f", Double(jsonFileSize) / Double(binaryFileSize)))x (binary smaller)")
