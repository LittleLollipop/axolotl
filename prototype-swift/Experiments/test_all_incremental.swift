...
class PersistentGraph {
    ...
    // MARK: - Connected Components (Weakly Connected)
...
func testAllIncrementalAlgorithms() {
    ...
    do {
        let filePath = "/tmp/test_all_incremental.json"
        try? FileManager.default.removeItem(atPath: filePath)
        ...
        // Test 2: Connected Components (Weakly Connected)
        print("\n📊 Test 2: Incremental Connected Components (Weakly Connected)")
        ...
        assert(components1.count == 2) // v0-v1-v2-v3-v4 are connected, v5 is isolated
        print("   ✅ Connected components test passed (2 components: one with 5 vertices, one with 1 vertex)")
        ...
        print("   Expected: 2 triangles (v0-v1-v2 and v1-v2-v3)")
        ...
        // Now v5 is connected to the main component
        let components2 = graph.connectedComponents()
        print("   Connected components after adding edge: \(components2.count)")
        assert(components2.count == 1) // All vertices should now be in one component
        print("   ✅ Connected components incremental update passed (now 1 component)")

        // Triangle count should still be 2
        let triangleCount2 = graph.triangleCount()
        print("   Triangle count after adding edge: \(triangleCount2)")
