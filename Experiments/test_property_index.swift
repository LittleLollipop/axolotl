...
print("=== Axolotl Graph Database - Property Index Test ===\n")

let testDbPath = "/tmp/test_graph_index.axolotl"

// Test 1: Add vertices with properties
print("Test 1: Add vertices with properties")
...
do {
    ...
    let v1 = try db.addVertex(properties: ["name": .string("Alice"), "age": .int(30), "active": .bool(true)])
    let v2 = try db.addVertex(properties: ["name": .string("Bob"), "age": .int(25), "active": .bool(true)])
    let v3 = try db.addVertex(properties: ["name": .string("Charlie"), "age": .int(35), "active": .bool(false)])
    let v4 = try db.addVertex(properties: ["name": .string("Alice"), "age": .int(28), "active": .bool(true)]) // Duplicate name
    
    print("\n1. Added 4 vertices with properties")
    
    // Find by property
    let aliceVertices = db.findByProperty("name", value: .string("Alice"))
    print("\n2. Find vertices where name = 'Alice': \(aliceVertices)")
    
    if aliceVertices.count == 2 {
        print("   ✅ Found 2 vertices named 'Alice'")
    } else {
        print("   ❌ Expected 2 vertices, found \(aliceVertices.count)")
    }
    
    // Get property values
    let nameValues = db.getPropertyValues("name")
    print("\n3. Unique values for 'name' property: \(nameValues.count) values")
    print("   Values: \(nameValues)")
    
    if nameValues.count == 3 { // Alice, Bob, Charlie
        print("   ✅ Found 3 unique names")
    } else {
        print("   ❌ Expected 3 unique names, found \(nameValues.count)")
    }
...
// Test 2: Update vertex properties
print("Test 2: Update vertex properties")
print(String(repeating: "-", count: 50))
...
do {
    ...
    if let firstAlice = aliceVertices.first {
        print("\n1. Updating vertex \(firstAlice) (Alice -> Alicia)")
        
        // Update properties
        try db.updateVertex(id: firstAlice, properties: ["name": .string("Alicia"), "age": .int(31), "active": .bool(true)])
        ...
        if aliceAfter.count == 1 && aliciaAfter.count == 1 {
            print("   ✅ Property index updated correctly")
        } else {
            print("   ❌ Property index not updated correctly")
        }
...
do {
    ...
    // Find Bob
    let bobVertices = db.findByProperty("name", value: .string("Bob"))
    
    if let bob = bobVertices.first {
        print("\n1. Deleting vertex \(bob) (Bob)")
        
        try db.deleteVertex(id: bob)
        
        // Verify index is updated
        let bobAfter = db.findByProperty("name", value: .string("Bob"))
        
        print("   Vertices named 'Bob' after deletion: \(bobAfter.count) (expected: 0)")
        
        if bobAfter.count == 0 {
            print("   ✅ Property index updated correctly after deletion")
        } else {
            print("   ❌ Property index not updated after deletion")
        }
...
// Test 4: Load from disk and verify index
print("Test 4: Load from disk and verify index")
print(String(repeating: "-", count: 50))
...
do {
    ...
    let charlieVertices = db.findByProperty("name", value: .string("Charlie"))
    let bobVertices = db.findByProperty("name", value: .string("Bob"))
    
    print("\n1. After reload from disk:")
    print("   Vertices named 'Alice': \(aliceVertices.count) (expected: 1)")
    print("   Vertices named 'Alicia': \(aliciaVertices.count) (expected: 1)")
    print("   Vertices named 'Charlie': \(charlieVertices.count) (expected: 1)")
    print("   Vertices named 'Bob': \(bobVertices.count) (expected: 0)")
    
    if aliceVertices.count == 1 && aliciaVertices.count == 1 && charlieVertices.count == 1 && bobVertices.count == 0 {
