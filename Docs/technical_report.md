...
| 指标 | 增量 PageRank | 增量 BFS | 原因 |
|------|---------------|-----------|------|
| **加速比** | 244x | 80x | BFS 的增量更新涉及的顶点更多（传播更广） |
| **迭代次数** | 1 | 1 | 都很快收敛（图变化小） |
| **精度** | 很高（误差 < 1e-6） | 完全匹配 | BFS 是精确算法，PageRank 是迭代近似 |
...
伪代码：
  ...
  while !affectedSet.empty() {
      // GPU：并行更新 affectedSet 中所有顶点的距离
      updatedDistances = gpuUpdateBFS(affectedSet, distances, graph)
...
这个设计的关键是：
- **CPU 和 GPU 同时工作** - GPU 更新 BFS 时，CPU 可以准备下一轮的检查
- **动态负载均衡** - 受影响的顶点数量动态变化，GPU 每次处理的任务量不同
...
6. 💡 **统一内存架构** 确实提供了新的可能性，正确的算法设计需要按工作类型分工，而不是按数据特征分工
...
下一步工作是**将这种分工方式应用到更多算法**（如增量 SSSP、增量 Connected Components 等），以及**优化 EdgeBlock 参数**（block 大小、填充策略等）。
