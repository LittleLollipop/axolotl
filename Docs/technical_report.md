...
8. 💡 **统一内存架构** 确实提供了新的可能性，正确的算法设计需要按工作类型分工，而不是按数据特征分工
...
**增量 PageRank、增量 BFS、增量 SSSP、增量 Connected Components 的成功证明了**：
...
| **加速比** | 244x | 80x | 84x | 74x | CC 的 Union-Find 操作极快 |
| **迭代次数** | 1 | 1 | 1 | 1 | 都很快收敛（图变化小） |
| **精度** | 很高（误差 < 1e-6） | 完全匹配 | 完全匹配 | 完全匹配 | CC/ BFS/SSSP 是精确算法 |
| **图类型** | 无权图 | 无权图 | 带权图 | 无权图 | CC 和 BFS 类似 |
| **核心数据结构** | 数组 | 数组 | 数组 | Union-Find | CC 使用特殊数据结构 |

下一步工作是**将这种分工方式应用到更多算法**（如增量 Triangle Counting、增量 Betweenness Centrality 等），以及**优化 EdgeBlock 参数**（block 大小、填充策略等）。
