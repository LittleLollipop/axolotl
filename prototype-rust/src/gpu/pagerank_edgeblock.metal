...
/// 增量 PageRank（EdgeBlock 版本，优化，正确处理悬挂顶点）
///
/// 改进：传入出度数组，正确处理悬挂顶点
///
/// 参数：
...
/// - reverse_blocks: 扁平化的反向 EdgeBlock 数据（每个 block 是 34 个 u32）
/// - pr: 当前 PR 值
/// - new_pr: 输出：新的 PR 值
/// - out_degrees: 每个顶点的出度
...
kernel void pagerank_edgeblock_optimized(
    ...
    device const uint *reverse_blocks [[buffer(4)]],  // 扁平化数组，每个 block 34 个 u32
    ...
    device const float *dangling_contribution [[buffer(10)]],  // 悬挂顶点的贡献（均匀分布）
    ...
    new_pr[v] = base_score + damping_factor * (contribution + dangling_contribution[0]);
}
